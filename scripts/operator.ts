...
// Cloudflare login authorizes the named remote service binding.
export default {
  async fetch(request: Request, env: { OPERATOR_TOKEN: string; PAYMENT_OPERATOR: { inspectRequest(key: string): Promise<unknown>; reconcilePaid(key: string, hash: string): Promise<unknown> } }) {
    ...
    const stub = env.PAYMENT_OPERATOR
